...
def ridge(stim, resp, alpha, singcutoff=1e-10, normalpha=False, logger=ridge_logger, solver_dtype=None):
...
def ridge_corr_pred(Rstim, Pstim, Rresp, Presp, valphas, normalpha=False,
    ...
    for ua in ualphas:
        ...
        alpha_pred = (PVh * (S/(S**2+ua**2))) @ UR[:,selvox]
...
def ridge_corr(Rstim, Pstim, Rresp, Presp, alphas, normalpha=False, corrmin=0.2,
               singcutoff=1e-10, use_corr=True, logger=ridge_logger, solver_dtype=np.float32):
    """Uses ridge regression to find a linear transformation of [Rstim] that approximates [Rresp],
    ...
    for na, a in zip(nalphas, tqdm(alphas, desc='alphas', leave=False)):
        ...
        pred = mult_diag(D, PVh, left=False) @ UR ## Best (1.75 seconds to prediction in test)
...
def bootstrap_ridge(Rstim, Rresp, Pstim, Presp, alphas, nboots, chunklen, nchunks,
                    ...
                    use_corr=True, return_wt=True, logger=ridge_logger, solver_dtype=None):
    ...
    for bi in tqdm(range(nboots), desc='Ridge bootstraps', leave=False):
        ...
        Rcmats.append(Rcmat)
    del RRstim, PRstim, RRresp, PRresp
    ...
    if return_wt:
        ...
        Pstim = Pstim.astype(wt.dtype)
        pred = (Pstim @ wt)

        # Find prediction correlations
        if use_corr:
            pred = np.nan_to_num(pred)
            corrs = np.nan_to_num(np.array([np.corrcoef(Presp[:,ii], pred[:,ii].ravel())[0,1]
